.PHONY: build vendor test lint clean proto
...
proto:
	@for FILE in proto/*.proto; do \
	    protoc -I$$GOPATH -I ${PWD}/vendor --proto_path=${PWD} --micro_out=. --go_out=. $$FILE || exit 1; \
	    echo compiled: $$FILE; \
	done
...
test:
	GOFLAGS='-mod=vendor' go test -p=1 -v -coverprofile=coverage.out $$(go list ./... | grep -v /vendor/)
...
clean:
	@rm -f coverage.out coverage.html coverage.xml snapshot
